...
/**
 * Tests for the XMLProtocolInfo class.
 * 
 * @author Daniel
 */
public class XMLProtocolInfoTests {
	...
	@Test
	public void parseXML_allArgsTest() throws FileNotFoundException,
			XMLStreamException, FactoryConfigurationError,
			IllegalXmlFormatException {
		Assert.assertNotNull("res is not in the classpath - ask Daniel",
				getClass().getClassLoader().getResource("protInfo.xml"));
		ProtocolInfo protInfo = new XMLProtocolInfo(getClass().getClassLoader()
				.getResource("protInfo.xml").getFile());
		...
		Assert.assertEquals(
				"ECqPGroup(P-256)::0000000002010000001c766572696669636174756d2e61726974686d2e4543715047726f75700100000005502d323536",
				protInfo.getGq());
		Assert.assertEquals(1, protInfo.getWidth());
	}

	@Test(expected = IllegalXmlFormatException.class)
	public void parseXML_withMissingArgTest() throws FileNotFoundException,
			XMLStreamException, FactoryConfigurationError,
			IllegalXmlFormatException {
		Assert.assertNotNull(
				"res is not in the classpath - ask Daniel",
				getClass().getClassLoader().getResource(
						"protInfoMissingArg.xml"));
		new XMLProtocolInfo(getClass().getClassLoader()
				.getResource("protInfoMissingArg.xml").getFile());
	}
}