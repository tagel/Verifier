...
public class ECurveGroupElementTests {
	...
	private Point point = new Point(new IntegerFieldElement(new LargeInteger("0"),
	...
	public void mult_caseXequalsYnegTest() {
		...
		Point pointRet = new Point(new IntegerFieldElement(new LargeInteger("-1"),
				f192), new IntegerFieldElement(new LargeInteger("-1"), f192));
		ECurveGroupElement eCurveElementRet = new ECurveGroupElement(pointRet,
				eCurveGroup192);
		
		Assert.assertTrue(eCurveElementRet.equal(eCurveElement1.mult(eCurveElement2)));
	}
	...
	public void mult_caseXareDiffTest() {
		...
		ECurveGroupElement eCurveElementOut = eCurveElement2.mult(eCurveElement1);
		Assert.assertEquals(new LargeInteger("-2").mod(eCurveGroup192.getFieldOrder()), eCurveElementOut.getElement().getX().getElement());
		Assert.assertEquals(new LargeInteger("2").mod(eCurveGroup192.getFieldOrder()), eCurveElementOut.getElement().getY().getElement());
	}
	...
	public void mult_withTheOneTest() {
		...
		ECurveGroupElement eCurveElementOut = eCurveElement2.mult(eCurveElement1);
		Assert.assertEquals(new LargeInteger("-1").mod(eCurveGroup192.getFieldOrder()), eCurveElementOut.getElement().getX().getElement());
		Assert.assertEquals(new LargeInteger("1").mod(eCurveGroup192.getFieldOrder()), eCurveElementOut.getElement().getY().getElement());
	}
	...
	public void equal_falseDiffInGroupTest() {
		...
		Point point2 = new Point(new IntegerFieldElement(new LargeInteger("-1"),
				f256), new IntegerFieldElement(new LargeInteger("-1"), f256));
		ECurveGroupElement eCurveElement2 = new ECurveGroupElement(point2,
				eCurveGroup256);
		Assert.assertFalse(eCurveElement1.equal(eCurveElement2));
	}
