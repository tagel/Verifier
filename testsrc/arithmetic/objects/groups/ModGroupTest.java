...
public class ModGroupTest {
	...
	private byte[] b = {0, 0, 0, 0, 3, 1, 0, 0, 0, 2, 1, 7, 1, 0, 0, 0, 2, 0, -125, 1, 0, 0, 0, 1, 2};
	...
	public void toByteArrayTest() throws UnsupportedEncodingException {
		Assert.assertEquals(Arrays.toString(b), Arrays.toString(group1.toByteArray()));
		ModGroup group2 = new ModGroup(b);
		Assert.assertEquals(Arrays.toString(b), Arrays.toString(group2.toByteArray()));
	}
	...
	public void getGeneratorTest() throws UnsupportedEncodingException {
		Assert.assertEquals(new LargeInteger("2"), group1.getGenerator().getElement());
		ModGroup group2 = new ModGroup(b);
		Assert.assertEquals(new LargeInteger("2"), group2.getGenerator().getElement());
	}
}
