...
public class IntegerFieldElementTests {
	...
	private PrimeOrderField pof_263 = new PrimeOrderField(new LargeInteger(263));
	...
	public void toByteArrayTest() {
		Assert.assertEquals("01000000020102", CryptoUtils
				.bytesToHexString(new IntegerFieldElement(
						new LargeInteger(258), pof_263).toByteArray()));

		System.out.println(CryptoUtils
				.bytesToHexString(new IntegerFieldElement(
						new LargeInteger(258), pof_263).toByteArray()));
		System.out.println("01000000020102");
		// Assert.assertEquals("01000000020005", CryptoUtils
		// .bytesToHexString(new IntegerFieldElement(new LargeInteger(5),
		// pof_263).toByteArray()));
	}

	// @Test
	// public void getElementTest() {
	// IntegerFieldElement ife = new IntegerFieldElement(new LargeInteger(1),
	// pof_4);
	// Assert.assertEquals(new LargeInteger(1), ife.getElement());
	// }
	//
	// @Test
	...
	//
	// ife = new IntegerFieldElement(new LargeInteger(1), pof_263);
	// Assert.assertTrue(pof_263.equals(ife.getField()));
	// }
	//
	// @Test
	// public void addTest() {
	// IntegerFieldElement ife1 = new IntegerFieldElement(
	// new LargeInteger(1), pof_4);
	// IntegerFieldElement ife2 = new IntegerFieldElement(
	// new LargeInteger(2), pof_4);
	// IntegerFieldElement ife3 = new IntegerFieldElement(
	// new LargeInteger(3), pof_4);
	...
	//
	// ife = new IntegerFieldElement(new LargeInteger(1), pof_4);
	// IntegerFieldElement ifeneg = new IntegerFieldElement(new LargeInteger(
	// 3), pof_4);
	// Assert.assertTrue(ife.neg().equal(ifeneg));
	//
	// ife = new IntegerFieldElement(new LargeInteger(5), pof_4);
	// ifeneg = new IntegerFieldElement(new LargeInteger(3), pof_4);
	//
	// Assert.assertTrue(ife.neg().equal(ifeneg));
	// }
	//
	...
	// new LargeInteger("3"), pof_4);
	//
	// Assert.assertTrue(ife3.equal(ife3.power(new LargeInteger("3"))));
	// Assert.assertTrue(ife2.getField().zero()
	// .equal(ife2.power(new LargeInteger("5"))));
	// }
	//
	// @Test
